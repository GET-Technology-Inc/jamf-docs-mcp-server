 ...
 * root). ci.yml's Test (Node 26.x) leg does that, and its Codecov Upload job
 * hands the directory to Codecov's bundle analyzer, which measures files on
 * disk and cannot see a bundle held as a JSON-escaped string inside a .ts
 * module. Same esbuild run, same bytes as the <script> element carries; the
 * generated module is identical either way.
 */
